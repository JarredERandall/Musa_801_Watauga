...
# 1. Introduction
Watauga County is located at the northwest corner of North Carolina. It is a mountainous area, with almost the entire county sitting high in the Appalachian Mountains. Boone is the largest Municipality in the county and is the home to Appalachian State University, a key economic driver for both the city and the county. The county has also become a popular vacation destination because of its rich natural landscape. 

![Watauga Location](https://raw.githubusercontent.com/JarredERandall/Musa_801_Watauga/main/Deliverable/FINAL/watauga_location.jpg)
...
```{r}
# read in data
prediction_0427 <- st_read("https://raw.githubusercontent.com/JarredERandall/Musa_801_Watauga/main/Modeling/0427/prediction_0427.geojson")
prediction_final <- st_read("https://raw.githubusercontent.com/JarredERandall/Musa_801_Watauga/main/Modeling/0427/prediction_final.geojson")
ZipCodes <- st_read("https://raw.githubusercontent.com/JarredERandall/Musa_801_Watauga/main/GeoJSON/ZipCodes.geojson")
ZipCodes <- st_transform(ZipCodes, st_crs(prediction_final))

# begin analysis
zip_joined <- st_join(prediction_final, ZipCodes, join = st_intersects)
zip_joined <- zip_joined %>%
              filter (developed == 0 & undevelopable == 0)
columns <- c("GlobalID", "Dvpt_Prob", "permit_lag", "GEOID20")
prediction_zips <- zip_joined[, columns]

prediction_zips <- prediction_zips %>%
                          mutate(predicted = ifelse(Dvpt_Prob > 0.5, 1, 0))
prediction_zips <- prediction_zips %>%
                          mutate(success = ifelse(predicted == permit_lag, 1,0))

prediction_zips_grouped <- prediction_zips %>%
  group_by(GEOID20) %>%
  summarize(successRate = sum(success == 1) / n())

# let's map!
mapping_data <- st_join(ZipCodes, prediction_zips_grouped, by = "GEOID20", all.x = TRUE)

ggplot(mapping_data) +
  geom_sf(aes(fill = successRate), color = "white") +
  geom_sf_text(aes(label = GEOID20.x), color = "black", size = 3) + 
  scale_fill_gradient(low = "white", high = "#e2a334", name = "Accuracy") +
  xlab(NULL) +
  ylab(NULL) +
  theme_void()
```
...
## 5.1 Users' Workflow
![Watauga Location](https://raw.githubusercontent.com/JarredERandall/Musa_801_Watauga/main/Deliverable/FINAL/workflow.jpg)

## 5.2 App Manual
![Watauga Location](https://raw.githubusercontent.com/JarredERandall/Musa_801_Watauga/main/Deliverable/FINAL/app_screenshot.png)
### Left Panel
