...
## 1.3 Dependent Variable: Septic System Permits 
...
path <- "E:/UPenn/24Spring/MUSA_Practicum/Musa_801_Watauga/Data/Landuse_Raster/"
...
path_1 <- "E:/UPenn/24Spring/MUSA_Practicum/Musa_801_Watauga/Data/slope_raster/"
...
dev.off()})
```
As 65%(≈33.02°) is the maximum slope suitbale for development, we will use 25° as a threshold to identify the areas unsuitable for developments.The plot below shows th reclassified slope degree in the county. The green areas are suitable for development, while the white areas are unsuitable for development.
...
## 3.3 Interacitve Map Development
The last step we plan to do is to develop an interactive map to visualize and enable the clients to interact with the data more easily. In the interactive map, we will include the data dashboard and the machine learning model prediction result on the map as suggestion for Watauga's future development.
