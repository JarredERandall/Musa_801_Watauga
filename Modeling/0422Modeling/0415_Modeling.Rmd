...
## 1. Loading Data
```{r}
# sample <- st_read("https://raw.githubusercontent.com/JarredERandall/Musa_801_Watauga/main/Resample/data_filtered.csv")
...
model_data <- resample%>%dplyr::select("permit_22", "permit_17.nn_3", "permit_17.nn_4", "permit_17.nn_5", "Slope_Ave", "Slope_Max", "Dist_Flowline", "Developed_Medium_Intensity", "Pasture_Hay", "Dist_Road", "Developed_High_Intensity", "Developed_Low_Intensity", "Developed_Open_Space")
```
...
train_data_scaled[, -which(names(train_data_scaled) == "permit_22")] <- scale(train_data_scaled[, -which(names(train_data_scaled) == "permit_22")])

# Standardize the predictor variables in the testing set
test_data_scaled <- test_data  # Create a copy of the testing data
test_data_scaled[, -which(names(test_data_scaled) == "permit_22")] <- scale(test_data_scaled[, -which(names(test_data_scaled) == "permit_22")])
```
...
for (ntree in ntrees) {
  for (mtry in mtrys) {
    for (min_node_size in min_node_sizes) {
      ...
      rf_model <- randomForest(permit_22 ~ ., data = train_data_scaled,
      ...
      # Calculate accuracy
      accuracy <- mean(rf_predictions == test_data_scaled$permit_22)
      
      # Calculate false negative rate
      actual_positives <- sum(test_data_scaled$permit_22 == 1)
      false_negatives <- sum(rf_predictions != test_data_scaled$permit_22 & test_data$permit_22 == 1)
...
best_rf_model <- randomForest(permit_22 ~ ., data = train_data_scaled,
...
conf_matrix <- confusionMatrix(data = rf_predictions, reference = test_data_scaled$permit_22)
...
# 5. Export Data (for Java)
```{r}
export_data <- resample%>%dplyr::select("GlobalID","permit_22", "permit_22", "permit_17.nn_3", "permit_17.nn_4", "permit_17.nn_5", "Slope_Ave", "Slope_Max", "Dist_Flowline", "Developed_Medium_Intensity", "Pasture_Hay", "Dist_Road", "Developed_High_Intensity", "Developed_Low_Intensity", "Developed_Open_Space", "area", "inflation17")
#export_data <- unique(export_data)
```

```{r}
rf_predictions <- predict(best_rf_model, newdata = model_data, type = "prob")
predictions_df <- as.data.frame(rf_predictions)

```

```{r}
export_data$"Dvpt_Prob" <- predictions_df$`0`
```
...
export_data $area_acre <- export_data$area / 43560
export_data <- export_data %>%
  rename("parcel_value17" = inflation17)
```


```{r}
merged_data <- merge(export_data, parcelid, by = "GlobalID", all.x = TRUE)
...
st_write(merged_data, "prediction_0422.geojson", driver = "GeoJSON")
```
``` {r}
